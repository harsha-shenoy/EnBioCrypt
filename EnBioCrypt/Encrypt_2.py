import io, random, time, os, way2sms, dropbox, gmail, tempfile, shutil, subprocess, sys, zlib, base64, hashlib, math, \
    gspread, pprint
...
def emver(email):
    ...
    while pol <= 3:
        ...
        msg = gmail.Message('EnbioCrypt - A New Level Encryption', to=email,
                            html="<center><img src='https://i.imgur.com/0evrOTf.jpg'/></center><br/><br/>Thanks For Using Our Service.<br/><br/>Your Verification Code is: <b>" + str(
                                mn) + "</b><br/><br/>With Regards,<br/><b>CEO Of EnBioCrypt</b>")
        gmtest.send(msg)
        ...
        try:
            ...
            if len(str(ver)) == 6 and ver == ver1:
                ...
                with open(dirpath1 + "/details.txt", "rb") as f21:
                    dbx.files_upload(f2.read(), '/EnBioCrypt/Logins/Emails/' + email + '/PassCode/' + str(ver) + '.txt',
                                     mute=True)
...
def nearp(sta):  # m2 global and m2 to be added
    global m2
    m2 = []
    # n=[]
    if sta >= 10:
...
def chekf(mo):
    while True:
        try:
            mo += str(random.randint(0, 100000000))
            if hashlib.md5(mo.encode('latin1')).hexdigest() not in hasher:
                if usr != 'n':
                    if not os.path.exists(tempfile.gettempdir() + "\\Dumps\\" + mo + ".png"):
                        return mo
                else:
                    r1 = dbx.files_get_metadata(
                        '/EnBioCrypt/Dumps/' + str(hashlib.md5(mo.encode('latin1')).hexdigest()) + '.png')
        except:
            return mo
                ...
                if polo == 'y' or polo == 'Y':
                    ...
                    with open("EncryptDumps/en" + r1.entries[chp].name, "wb") as f5:
                        metadata, res = dbx.files_download(
                            path='/EnBioCrypt/Logins/Emails/' + email + '/Storage(' + email + ')/' + r1.entries[
                                chp].name)
                        # print(res.content)
                        f5.write(res.content)
                    f5.close()
                    print("\n The Requested Encrypted File has been downloaded to EncryptDumps with name: en" +
                          r1.entries[chp].name)
                    metadata, res = dbx.files_download(
                        path='/EnBioCrypt/Logins/Emails/' + email + '/PrivateKeys/' + r1.entries[chp].name)
                    mcon = (res.content).decode('ascii')
                    msg = gmail.Message('EnbioCrypt - A New Level Encryption', to=email,
                                        html="<center><img src='https://i.imgur.com/0evrOTf.jpg'/></center><br/><br/>Thanks For Using Our Service.<br/><br/>Your Private KEy for File <b>en" +
                                             r1.entries[
                                                 chp].name + "</b> is: <b>" + mcon + "</b><br/><br/>With Regards,<br/><b>CEO Of EnBioCrypt</b>")
                    gmtest.send(msg)
                    ...
                    mcon = (res.content).decode('ascii')
                    msg = gmail.Message('EnbioCrypt - A New Level Encryption', to=email,
                                        html="<center><img src='https://i.imgur.com/0evrOTf.jpg'/></center><br/><br/>Thanks For Using Our Service.<br/><br/>Your Private KEy for File <b>" +
                                             r1.entries[
                                                 chp].name + "</b> is: <b>" + mcon + "</b><br/><br/>With Regards,<br/><b>CEO Of EnBioCrypt</b>")
                    gmtest.send(msg)
                    w2s.send(mobno, "\nCheck mail for file,sender's info.\n\nFile: " + r1.entries[
            ...
            if chp <= len(r1.entries) and chp > 0:
                ...
                if ipl == 'y' or ipl == 'Y':
                    result = dbx.files_get_temporary_link(
                        '/EnBioCrypt/Logins/Emails/' + email + '/Storage(' + email + ')/' + r1.entries[chp].name)
                else:
                    result = dbx.sharing_create_shared_link(
                        '/EnBioCrypt/Logins/Emails/' + email + '/Storage(' + email + ')/' + r1.entries[chp].name,
                        short_url=True, pending_upload=None)
                ...
                w2s.send(sphn, "\nCheck mail for file,sender's info.\n\nFile: " + sup1 + "\n\nPrivate Key(" + str(
                    len(mcon)) + "Chars):\n\n" + mcon + "\n\n-Made With Love From EnBioCrypt.")
                if ipl == 'y' or ipl == 'Y':
                    msg = gmail.Message('EnbioCrypt - A New Level Encryption', to=smil,
                                        html="<center><img src='https://i.imgur.com/0evrOTf.jpg'/></center><br/><br/>Thanks For Using Our Service.<br/>The Encrypted File has been uploaded in our storage space , <b>The link for Your Encrypted File is:</b><br/><br/>Sender's Name: <b>" + nam + "<br/><br/></b>File Name: <b>en" +
                                             r1.entries[
                                                 chp].name + "</b><br/><br/><b>" + result.link + "</b><br/><br/>The Private Key Has been/will be sent to <b>Mobile Phone: +91xxxxxx" + sphn[
                                                                                                                                                                                       6:] + "</b><br/><br/><b>NOTE:</b>The download link will be only available for limited time,<b>Please be sure to download ASAP(Within 4 Hours)</b>.<br/><br/>With Regards,<br/><b>CEO Of EnBioCrypt</b>")
                else:
                    msg = gmail.Message('EnbioCrypt - A New Level Encryption', to=smil,
                                        html="<center><img src='https://i.imgur.com/0evrOTf.jpg'/></center><br/><br/>Thanks For Using Our Service.<br/>The Encrypted File has been uploaded in our storage space , <b>The link for Your Encrypted File is:</b><br/><br/>Sender's Name: <b>" + nam + "<br/><br/></b>File Name: <b>en" +
                                             r1.entries[
                                                 chp].name + "</b><br/><br/><b>" + result.url + "</b><br/><br/>The Private Key Has been/will be sent to <b>Mobile Phone: +91xxxxxx" + sphn[
                                                                                                                                                                                      6:] + "</b><br/><br/><b>NOTE:</b>The download link will be only available for limited time,<b>Please be sure to download ASAP(Within 4 Hours)</b>.<br/><br/>With Regards,<br/><b>CEO Of EnBioCrypt</b>")
                gmtest.send(msg)
        ...
        try:
            ...
            def chekf():
                ...
                while (True):
                    try:
                        r1 = dbx.files_get_metadata(
                            '/EnBioCrypt/Logins/Emails/' + email + '/Storage(' + email + ')/en' + mpc + '.txt')
            ...
            with open(dirpath + "\en" + moc + ".txt", "rb") as f2:
                dbx.files_upload(f2.read(),
                                 '/EnBioCrypt/Logins/Emails/' + email + '/Storage(' + email + ')/en' + moc + '.txt',
                                 mute=True)
            ...
            msg = gmail.Message('EnbioCrypt - A New Level Encryption', to=rector,
                                html="<center><img src='https://i.imgur.com/0evrOTf.jpg'/></center><br/><br/>Thanks For Using Our Service.<br/>The Encrypted File has been uploaded in our storage space , <b>The link for Your Encrypted File is can be manually generated.</b><br/><br/>File Name: <b>en" + moc + ".txt</b> has been uploaded successfully.<br/><br/>The Private Key Has been/will be sent to <b>Mobile Phone: +91xxxxxx" + mobno[
                                                                                                                                                                                                                                                                                                                                                                                                                                              6:] + "</b><br/><br/>With Regards,<br/><b>CEO Of EnBioCrypt</b>")
            gmtest.send(msg)
            finstr = ""
            ...
            with open(dirpath + "/test.txt", "rb") as f00:
                dbx.files_upload(f00.read(), '/EnBioCrypt/Logins/Emails/' + email + '/PrivateKeys/en' + moc + '.txt',
                                 mute=True)
            f00.close()
...
def act5(email):
    ...
    sl = dbx.sharing_create_shared_link('/EnBioCrypt/Logins/Emails/' + email + '/Storage(' + email + ')',
                                        short_url=True, pending_upload=None)
    msg = gmail.Message('EnbioCrypt - A New Level Encryption', to=em,
                        html="<center><img src='https://i.imgur.com/0evrOTf.jpg'/></center><br/><br/>Thanks For Using Our Service.<br/><br/>Your Storage Link is: <b>" + sl.url + "</b><br/><br/><b>NOTE: </b>The Above Storage is Private Link Which will ONLY contain Your Encrypted Files,<b>The LINK IS PRIVATE, share it with your own RISK</b>,Private Keys can only be obtained On Mobile that to only when you Request it.<br/><br/>With Regards,<br/><b>CEO Of EnBioCrypt</b>")
    gmtest.send(msg)
    w2s.send(str(pn),
...
# Expt
    ...
    while True:
        ...
        no = int(siz.st_size / (siz.st_size / 10))
        while cou < no:
            moc = chekf(mo)
        ...
        for i in tqdm(range(no)):
            ...
            finstr = ''.join(bytes([int(i)]).decode('latin1') for i in m2)
            pv.append('"' + finstr + '"')
            finstr2 = hashlib.md5(finstr.encode('latin1')).hexdigest()
            ...
            for i in range(len(m)):
                ...
                if h >= 32:
                    h = 0
                numbers = "{0:b}".format(
                    exfun(sum(bytearray(hashlib.md5(finstr2[z].encode('latin1')).hexdigest(), 'latin1')))).zfill(8)
                if k >= len(m2):
                    k = 0
            ...
            im1.putpixel((0, 0), (1, int(ter / 4) + get(ter % 4), ter, len(po)))
        ...
        while True:
            ...
            if (x, y) in ress:
                ...
                im1.putpixel((x, y), (
                random.randint(0, 256), random.randint(0, 256), random.randint(0, 256), random.randint(0, 256)))
            y += 1
        ...
        for x in range(len(emails)):
            msg = gmail.Message('EnbioCrypt - A New Level Encryption', to=emails[x],
                                html="<center><img src='https://i.imgur.com/0evrOTf.jpg'/></center><br/><br/>Thanks For Using Our Service.<br/>The Encrypted File has been uploaded in our storage space , <b>The Generated EBC card Image Has been Attached with this mail.</b><br/><br/>Sender's Name: <b>" + kter + "<br/><br/></b>File Name: <b>" + mo + ".png</b><br/><br/>Further OTP's will be sent at the time of verification to <b>Mobile Phone: +91xxxxxx" +
                                     mobiles[x][
                                     6:] + "</b><br/><br/><b>NOTE:</b>The Attached Image can be used to Fetch the data from cloud after providing successful Identity Verification , <b> Make Sure to have Decrypter</b>.<br/><br/>With Regards,<br/><b>CEO Of EnBioCrypt</b>",
                                attachments=[mo + '.png'])
            gmtest.send(msg)
